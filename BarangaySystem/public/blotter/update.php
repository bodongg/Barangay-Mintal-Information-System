...
try {
  $stmt = db()->prepare("SELECT * FROM blotter_records WHERE id = :id LIMIT 1");
  $stmt->execute(['id' => $id]);
  $row = $stmt->fetch();
  if (!$row) {
    header('Location: /BarangaySystem/public/blotter/index.php');
  ...
  $form = [
    ...
    'blotter_type' => (string) $row['blotter_type'],
    'contact_number' => (string) $row['contact_number'],
    'address' => (string) $row['address'],
    'incident_details' => (string) ($row['incident_details'] ?? ''),
  ...
  if (count($errors) === 0) {
    try {
      ...
      $stmt->execute([
        'id' => $id,
        'name' => $form['name'],
        'blotter_type' => $form['blotter_type'],
        'contact_number' => $form['contact_number'],
        'address' => $form['address'],
        'incident_details' => $form['incident_details'] === '' ? null : $form['incident_details'],
        'date' => $form['date'],
        'purok' => $form['purok'],
        'status' => $form['status'],
      ]);
      header('Location: /BarangaySystem/public/blotter/index.php?msg=updated');
      exit;
    } catch (Throwable $e) {
      $errors[] = 'Failed to update blotter report. Check database setup.';
    }
  }
}
?>
  ...
  <div class="content">
    <div class="card">
      <div class="table-header">
        <div>
          <div class="card-title">Edit Blotter Report</div>
          <div class="card-sub">Update incident details and status</div>
        </div>
