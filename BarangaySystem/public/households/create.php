...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (count($errors) === 0) {
    try {
      $stmt = db()->prepare("
        INSERT INTO households (household_code, head_resident_id, purok, address, housing_type, status)
        VALUES (:household_code, :head_resident_id, :purok, :address, :housing_type, :status)
      ");
      $stmt->execute([
      ...
      header('Location: /BarangaySystem/public/households/index.php?msg=saved');
      exit;
    } catch (Throwable $e) {
      $errors[] = 'Failed to save household. Check database setup.';
    }
  }
}
?>
    ...
    <div class="card">
      <div class="table-header">
        <div>
          ...
          <div class="card-sub">Create a new household profile</div>
        </div>
